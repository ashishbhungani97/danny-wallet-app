...
function add(data) {
  return async dispatch => {
    dispatch(
      RequestConstant(TransactionConstant.ADD_TRANSACTION_REQUEST, data),
    );
    const result = await TransactionService.add(data);
    dispatch(
      ResponseConstant(
        TransactionConstant.ADD_TRANSACTION_SUCCESS,
        TransactionConstant.ADD_TRANSACTION_FAILURE,
        result,
      ),
    );
    return result;
...
function list(address) {
  return async dispatch => {
    ...
    transactions.map(transaction => {
      transaction.sentOrReceived =
        transaction.from.toUpperCase() == address.toUpperCase()
          ...
          : 'Received';
      transaction.status =
        transaction.txreceipt_status == '0' ? 'Pending' : 'Confirmed';
    ...
    dispatch(
      ResponseConstant(
        TransactionConstant.LIST_TRANSACTION_SUCCESS,
        TransactionConstant.LIST_TRANSACTION_FAILURE,
        result,
      ),
    );
    return result;
