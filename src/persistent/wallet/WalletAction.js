...
async function setWallets(wallet) {
  const wallets =
    (await LMStorageService.getItem(LMStorageConstant.WALLETS_STORAGE_KEY)) ||
    [];
  wallets.push(wallet);
  await LMStorageService.setItem(
    LMStorageConstant.WALLETS_STORAGE_KEY,
    wallets,
  );
}
...
function getActiveWallet() {
  return async dispatch => {
    const {privateKey, name} = await LMStorageService.getItem(
      LMStorageConstant.ACTIVE_WALLET_STORAGE_KEY,
    );
    const {success, data} = await WalletService.fromPrivateKey(privateKey);
    const wallet = {
