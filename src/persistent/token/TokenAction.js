...
function getTokens({chainId}) {
  return async dispatch => {
    ...
    for (const [key, value] of Object.entries(
      ApplicationProperties.TOKEN_URLS,
    )) {
...
function filterTokens(keyword, chainId) {
  return async dispatch => {
    if (keyword.trim() == '') {
      ...
      const tokens = await LMStorageService.getItem(
        LMStorageConstant.TOKENS_STORAGE_KEY,
      );
      const data = _.remove(tokens, function (token) {
        return (
          token.name.includes(keyword) ||
          token.symbol.includes(keyword) ||
          _.toUpper(token.address).includes(keyword)
        );
      });
      dispatch(getTokensSuccess(data));
    }
