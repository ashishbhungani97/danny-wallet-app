...
function addAsset(wallet, chainId, token) {
  return async dispatch => {
    ...
    const success = await AssetService.addAsset(wallet.address, chainId, {
      ...token,
      balance,
    });
    if (success) {
...
function removeAsset(wallet, chainId, token) {
  return async dispatch => {
    const assets = await AssetService.removeAsset(
      wallet.address,
      chainId,
      token,
    );
...
function list(wallet, chainId) {
  return async dispatch => {
    ...
    for (let i = 0; i < assets.length; i++) {
      assets[i].balance = await PancakeswapModule.tokenBalance(
        wallet,
        assets[i],
      );
    }
