...
} from '../../libs/simple-pancakeswap-sdk';
import EtherUtilModule from '../etherjs/EtherUtilModule';
import {BigNumber, utils} from 'ethers';
import _ from 'lodash';
import {BNB} from '../../libs/simple-pancakeswap-sdk/common/tokens';
...
const swap = async (ethWallet, trade) => {
  ...
  try {
    if (trade.approvalTransaction) {
      const approved = await ethWallet.sendTransaction(
  ...
  try {
    if (!_.isNil(token.isDAN)) {
      ...
      const tokenFactoryPublic = new TokenFactoryPublic(
        ...
        'https://mainnet.danscan.io/',
      );
      const ethereumAddress = wallet.address;
      const hex = await tokenFactoryPublic.balanceOf(ethereumAddress);
      const formatEther = EtherUtilModule.formatEther(
...
const commonToken = async wallet => {
  ...
  ethWallet.name = 'DANNY';
  ethWallet.symbol = 'DAN';
  ethWallet.logoURI =
    'https://alchemy.mypinata.cloud/ipfs/bafkreifp7a2j7tpyqvvdyw5go234t6yabkkhmrckiqovffbpcs2272u6ui';
  return ethWallet;
};
const getToken = async tokenAddress => {
  try {
    ...
    const tokens = await tokensFactoryPublic.getTokens([tokenAddress]);
    return tokens[0];
  } catch (e) {
    return e.message;
  }
