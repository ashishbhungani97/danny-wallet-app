...
export default function TransferScreen({navigation, route, lang}) {
  ...
  const schema = yup.object().shape({
    recipientAddress: yup
      .string()
      .required(lang.pleaseInputRecipientAddress)
      .isAddress(lang.addressIsIncorrect),
    amount: yup
      .number()
      .positive(lang.shouldBePositiveNumber)
      .typeError(lang.shouldBeANumber),
  });
  ...
  useEffect(() => {
    async function fetchGasTracker() {
      const nextGasTracker = await WalletService.getFeeSuggestions(gasLimit);
      setGasTracker(nextGasTracker);
      setSelectedGas(nextGasTracker.proposeGasPrice);
    }
    fetchGasTracker();
  }, [gasLimit]);
            ...
            <>
              ...
              <View style={styles.operationContainer}>
                <TouchableOpacity
                  ...
                  {selectedGas.key == 3 && (
                    <LMGradient style={styles.selected} />
                  )}
                  <View style={styles.itemContent}>
                    <Text style={{fontWeight: 'bold', color: white}}>
                      {lang.fast}
