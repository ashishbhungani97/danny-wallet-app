...
export default function ChangePasswordScreen({navigation, route, lang}) {
  ...
  const schema = yup.object().shape({
    currentPassword: yup
      .string()
      .required(lang.pleaseCurrentPassword)
  ...
  const onSubmit = async ({currentPassword, password, confirmPassword}) => {
    if (user.password != currentPassword) {
      ...
      dispatch(
        UserAction.updateProfile({currentPassword, password, confirmPassword}),
      ).then(() => {
    ...
    <Root>
      <SafeAreaView style={styles.container}>
        ...
        <View style={[styles.buttonsContainer, {marginBottom: 5}]}>
          <LMButton label={lang.save} onPress={handleSubmit(onSubmit)} />
        </View>
      </SafeAreaView>
    </Root>
  );
