...
export default function PancakeSwapScreen({navigation}) {
  ...
  const init = async () => {
    setFromToken(await PancakeswapModule.commonToken(rawActiveWallet));
  };
  const reset = async () => {
    //reset fromToken
    const token = {...fromToken};
    token.balance = await PancakeswapModule.tokenBalance(
      rawActiveWallet,
      fromToken,
    );
    setFromToken(token);
    ...
    token2.balance = await PancakeswapModule.tokenBalance(
      rawActiveWallet,
      token2,
    );
    setToToken(token2);
  ...
  const exchange = async (from, to, value) => {
    if (isValidTrade(value)) {
      setError('Loading...');
      const finalValue = value;
      const exchange = await PancakeswapModule.trade(
  ...
  const swap = async () => {
    LMLoading.show();
    const result = await PancakeswapModule.swap(rawActiveWallet, trade);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.contentContainer}>
                ...
                onPress={() => {
                  navigation.navigate('TokenScreen', {
                    onSelect: async token => {
                      const balance = await PancakeswapModule.tokenBalance(
                        rawActiveWallet,
                        token,
                      );
              ...
              <View
                ...
                <View style={[styles.half, {paddingLeft: 5}]}>
                  ...
                  <LMTouchableOpacity
                    ...
                    <Text
                      style={{color: 'rgb(232, 0, 111)', fontWeight: 'bold'}}>
                      {' '}
                      ({language.maximum})
                    </Text>
                  </LMTouchableOpacity>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.half}>
              <View
                ...
                <View style={[styles.half, {paddingLeft: 5}]}>
                  <Text style={{color: TEXT_SECONDARY}}>{toToken.symbol}</Text>
                </View>
              </View>
            </View>
          </View>
        ...
        <View style={styles.exchangeRate}>
          <Text style={{color: TEXT_SECONDARY}}>
            PancakeSwap {trade.uniswapVersion}
          </Text>
          <View
            style={{
              alignItems: 'center',
        ...
        <View style={{marginTop: 0}}>
          <LMButton
            label={error}
            backgroundColor={error == 'Swap' ? undefined : red}
            onPress={async () => {
              if (error == 'Swap') {
                await swap();
              }
        ...
        <View style={styles.transaction}>
          <View style={styles.transactionRow}>
            <Text style={{color: TEXT_SECONDARY}}>{language.route}</Text>
            <Text style={{color: TEXT_SECONDARY}}>{trade.routeText}</Text>
          </View>
          <View style={styles.transactionRow}>
            <Text style={{color: TEXT_SECONDARY}}>
              {language.minimumedReceived}
            </Text>
            <NumberFormat
              ...
              thousandSeparator={true}
              decimalScale={4}
              renderText={value => <Text style={{color: white}}>{value}</Text>}
            />
          </View>
          <View style={styles.transactionRow}>
            <Text style={{color: TEXT_SECONDARY}}>
              {language.slippageTolerance}
            </Text>
            <Text style={{color: TEXT_SECONDARY}}>0.05%</Text>
          </View>
        </View>
      </View>
    </SafeAreaView>
