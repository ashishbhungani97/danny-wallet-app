...
const hexlify_1 = require('../../common/utils/hexlify.js');
const parse_ether_1 = require('../../common/utils/parse-ether.js');
const to_ethers_big_number_1 = require('../../common/utils/to-ethers-big-number.js');
const trade_path_1 = require('../../common/utils/trade-path.js');
const trade_path_2 = require('../../enums/trade-path.js');
const pancakeswap_router_contract_factory_1 = require('../router/pancakeswap-router-contract.factory.js');
const pancakeswap_router_factory_1 = require('../router/pancakeswap-router.factory.js');
const token_factory_1 = require('../token/token.factory.js');
const pancakeswap_pair_contract_factory_1 = require('./pancakeswap-pair-contract.factory.js');
class PancakeswapPairFactory {
  constructor(_pancakeswapPairFactoryContext) {
    ...
    this._fromTokenFactory = new token_factory_1.TokenFactory(
      this._pancakeswapPairFactoryContext.fromToken.contractAddress,
      this._pancakeswapPairFactoryContext.ethersProvider,
    );
    this._pancakeswapRouterContractFactory =
      new pancakeswap_router_contract_factory_1.PancakeswapRouterContractFactory(
        this._pancakeswapPairFactoryContext.ethersProvider,
      );
    this._pancakeswapPairFactory =
      new pancakeswap_pair_contract_factory_1.PancakeswapPairContractFactory(
        this._pancakeswapPairFactoryContext.ethersProvider,
      );
    this._pancakeswapRouterFactory =
      new pancakeswap_router_factory_1.PancakeswapRouterFactory(
  ...
  async executeTradePath(amount) {
    switch (this.tradePath()) {
      ...
      default:
        throw new __1.PancakeswapError(
          `${this.tradePath()} is not defined`,
          error_codes_1.ErrorCodes.tradePathIsNotSupported,
        );
    }
  }
  /**
  ...
  async trade(amount) {
    this.destroy();
    const tradeContext = await this.executeTradePath(
  ...
  async findBestRoute(amountToTrade) {
    return await this._routes.findBestRoute(
      new bignumber_js_1.default(amountToTrade),
    );
  }
  /**
   * Find the best route rate out of all the route quotes
  ...
  _hasGotEnoughAllowance(amount, allowance) {
    ...
    const bigNumberAllowance = new bignumber_js_1.default(allowance).shiftedBy(
  ...
  hasGotEnoughBalanceErc20(amount, balance) {
    const bigNumberBalance = new bignumber_js_1.default(balance).shiftedBy(
      this.fromToken.decimals * -1,
    );
  ...
  async hasGotEnoughBalanceEth(amount) {
    ...
    const bigNumberBalance = new bignumber_js_1.default(balance).shiftedBy(
      constants_1.Constants.ETH_MAX_DECIMALS * -1,
    );
    if (new bignumber_js_1.default(amount).isGreaterThan(bigNumberBalance)) {
      return {
        hasEnough: false,
  ...
  async generateApproveMaxAllowanceData() {
    if (this.tradePath() === trade_path_2.TradePath.ethToErc20) {
      throw new __1.PancakeswapError(
        ...
        error_codes_1.ErrorCodes.generateApproveMaxAllowanceDataNotAllowed,
      );
    }
  ...
  async findBestPriceAndPathErc20ToEth(erc20Amount) {
    ...
    const convertQuoteWithSlippage = new bignumber_js_1.default(
  ...
  generateTradeDataErc20ToEth(
    ...
    const amountIn = tokenAmount
      ...
      .decimalPlaces(0);
    const ethAmountOutWei = (0, hexlify_1.hexlify)(
      (0, parse_ether_1.parseEther)(ethAmountOutMin),
    );
    return this._pancakeswapRouterContractFactory.swapExactTokensForETH(
      (0, hexlify_1.hexlify)(amountIn),
      ethAmountOutWei,
      routePathArray,
      this._pancakeswapPairFactoryContext.ethereumAddress,
      deadline,
  ...
  buildUpTransactionEth(ethValue, data) {
    return {
      ...
      from: this._pancakeswapPairFactoryContext.ethereumAddress,
      data,
      value: (0, to_ethers_big_number_1.toEthersBigNumber)(
        (0, parse_ether_1.parseEther)(ethValue),
      ).toHexString(),
    };
  }
  /**
    ...
    this._quoteChangeTimeout = setTimeout(async () => {
      if (this._quoteChanged$.observers.length > 0) {
        const trade = await this.executeTradePath(
          new bignumber_js_1.default(tradeContext.baseConvertRequest),
        );
        if (
          !new bignumber_js_1.default(trade.expectedConvertQuote).eq(
