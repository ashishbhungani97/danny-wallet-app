...
const bnb_1 = require('../../common/tokens/bnb.js');
const comp_1 = require('../../common/tokens/comp.js');
const dai_1 = require('../../common/tokens/dai.js');
const usdc_1 = require('../../common/tokens/usdc.js');
const usdt_1 = require('../../common/tokens/usdt.js');
const format_ether_1 = require('../../common/utils/format-ether.js');
const hexlify_1 = require('../../common/utils/hexlify.js');
const only_unique_1 = require('../../common/utils/only-unique.js');
...
const router_direction_1 = require('./enums/router-direction.js');
class PancakeswapRouterFactory {
  constructor(_fromToken, _toToken, _disableMultihops, _ethersProvider) {
    ...
    this._multicall = new ethereum_multicall_1.Multicall({
      ...
      MulticallOptionsCustomJsonRpcProvider : "https://mainnet.danscan.io"
    });
  }
  /**
    ...
    const fromTokenRoutes = {
      ...
      pairs: {
        fromTokenPairs: this.getTokenAvailablePairs(
          this._fromToken,
          availablePairs,
          router_direction_1.RouterDirection.from,
        ),
      },
    };
    const toTokenRoutes = {
      ...
      pairs: {
        toTokenPairs: this.getTokenAvailablePairs(
          this._toToken,
          availablePairs,
          router_direction_1.RouterDirection.to,
        ),
      },
    };
    const allMainRoutes = [];
  ...
  async findBestRoute(amountToTrade) {
    ...
    if (allRoutes.length === 0) {
      throw new pancakeswap_error_1.PancakeswapError(
        ...
        error_codes_1.ErrorCodes.noRoutesFound,
      );
    }
    return {
      ...
      if (
        ...
        for (let f = 0; f < fromTokenRoutes.pairs.fromTokenPairs.length; f++) {
          ...
          if (
            ...
            if (
              workedOutFromRoute.filter(only_unique_1.onlyUnique).length ===
        ...
        for (let f = 0; f < toTokenRoutes.pairs.toTokenPairs.length; f++) {
          ...
          if (
            tokenRoute.pairs.fromTokenPairs.find(
              pair => pair.contractAddress === toSupportedToken.contractAddress,
            )
          ) {
            ...
            if (
              workedOutToRoute.filter(only_unique_1.onlyUnique).length ===
  ...
  buildRouteQuotesFromResults(contractCallReturnContext) {
    ...
    if (contractCallReturnContext) {
      ...
      return result.sort((a, b) => {
        if (
          new bignumber_js_1.default(a.expectedConvertQuote).isGreaterThan(
            b.expectedConvertQuote,
          )
        ) {
          return -1;
        }
        return new bignumber_js_1.default(a.expectedConvertQuote).isLessThan(
          b.expectedConvertQuote,
        )
          ? 1
          : 0;
  ...
  buildRouteQuoteForEthToErc20(callReturnContext) {
    const convertQuoteUnformatted = new bignumber_js_1.default(
  ...
  buildRouteQuoteForErc20ToEth(callReturnContext) {
    ...
    return {
      expectedConvertQuote: new bignumber_js_1.default(
        (0, format_ether_1.formatEther)(convertQuoteUnformatted),
      ).toFixed(this._toToken.decimals),
      routePathArrayTokenMap: callReturnContext.methodParameters[1].map(c => {
        return this.allTokens.find(t => t.contractAddress === c);
      }),
  ...
  formatAmountToTrade(amountToTrade) {
    switch (this.tradePath()) {
      ...
      case trade_path_2.TradePath.erc20ToEth:
      case trade_path_2.TradePath.erc20ToErc20:
        return (0, hexlify_1.hexlify)(
          amountToTrade.shiftedBy(this._fromToken.decimals),
        );
      default:
        throw new pancakeswap_error_1.PancakeswapError(
  ...
  get allMainTokens() {
    if (
      this._ethersProvider.provider.network.chainId === chain_id_1.ChainId.BSC
    ) {
      return [
